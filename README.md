...
to store the key in `.env.local` instead. If Claude is already configured, use
`scripts/ninoctl finish --skip-configure` to restart, audit, write evidence and
print the completion matrix without pasting the key again. That mode fails fast
if Claude is not already present in `.env.local` or the current environment. The
flow also writes a fresh closing report and prints the completion audit so the
final evidence matches the audited revision.
