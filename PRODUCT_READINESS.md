...
| Claude opcional | `scripts/ninoctl configure-claude`, `scripts/nino-configure-claude`, `.env.local` o macOS Keychain, `NINO_LLM_PROVIDER=claude`, `/operations/claude`, `POST /operations/claude/configure`, panel LLM con `Guardar Claude` y modo Keychain por defecto, `/llm/probe`, `config_errors` para valores invalidos | Implementado; prueba viva requiere key real |
