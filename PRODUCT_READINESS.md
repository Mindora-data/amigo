...
| Operacion local | `scripts/ninoctl`, `scripts/ninoctl status` y `logs` conscientes de launchd, `scripts/ninoctl wait-health` con detalle de error `curl`, `scripts/nino-install-local`, `scripts/nino-launchd`, `scripts/nino-launchd doctor`, `scripts/ninoctl persistent-audit`, `GET /operations/logs`, `POST /operations/restart`, panel Logs y reinicio en UI, `nino-server`, `nino-smoke` | Cumplido |
...
| Informe de cierre | `scripts/ninoctl closing-report`, `scripts/ninoctl reports`, `scripts/ninoctl report`, `scripts/ninoctl report latest`, `nino-closing-report`, `POST /operations/closing-report`, `GET /operations/reports`, `GET /operations/reports/latest`, `GET /operations/reports/{report_name}`, botones `Informe cierre`, `Ver informes`, `Último informe` y `Ver JSON`, smoke `closing_report`, `closing_report_list`, `closing_report_read`, `closing_report_latest`, `closing_report_name_guard` y `next_action`, JSON en `data/reports/` | Cumplido local |
...
o en JSON con `--json`. `scripts/ninoctl next-action` imprime solo el proximo
comando operativo. Tambien marca si ese informe corresponde a la revision actual;
