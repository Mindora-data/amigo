...
- Rama activa: `main`.
- Suite actual: 129 tests pasando.
...
28. Hecho inicial: configuracion y desactivacion de Claude desde API/UI con `POST /operations/claude/configure`, `POST /operations/claude/disable`, botones `Guardar Claude` y `Desactivar Claude`, sin devolver la key, con Keychain por defecto o `.env.local` con modo `600`.
